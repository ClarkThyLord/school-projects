...
<!-- CANDIDATES NOTES MODIFY -->
<div class="modal fade" id="candidates_notes_modify" role="dialog" aria-labelledby="jobs_edit" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
			<!-- HEADER -->
      <div class="modal-header">
        <h5 class="modal-title">Modifica Candidato</h5>
          ...
          <span aria-hidden="true">&times;</span>
			...
			<!-- BODY -->
      <div class="modal-body">
        <form action="#" data-current-step="0" id="candidates_notes_modify_info">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Cerrar</button>
        <button type="button" onclick="(async function () { var data = await html_to_data($('#candidates_data_modify_info').first()[0]); if (!data) { return; } candidates_modify(GLOBALS.asset.id || alert('¡Algo salió mal!'), {'name': $('#candidates_data_modify_info :input[data-backup=\'Nombre Completo\']').val(), data: data, 'notes': $('#candidates_notes_modify_info :input[name=\'notes\']').val()}); $('#candidates_notes_modify').modal('hide'); $('#candidates_data_modify').modal('hide').find('form').trigger('reset'); })();" class="btn btn-primary">Someter</button>
      </div>
    </div>
  </div>
</div>
