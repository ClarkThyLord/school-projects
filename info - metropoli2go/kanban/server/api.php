...
// sections   | modify    | POST      | 1^
// landmarks  |           | GET       | none
// landmarks  | create    | POST      | 1^
// landmarks  | remove    | POST      | 1^
// landmarks  | modify    | POST      | 1^
// files      |           | GET       | none
// files      | create    | POST      | 1^
// files      | remove    | POST      | 1^
// logs       |           | GET       | none
	...
	if (count($routes) <= 0) {
		...
		if (count($routes) === 2){
			if ($_SERVER['REQUEST_METHOD'] === 'GET') {
				section_get(json_decode($_GET['filter'], true), json_decode($_GET['options'], true));
			} else if ($_SERVER['REQUEST_METHOD'] === 'POST') {
				switch ($routes[1]) {
					case 'add':
						section_add($_POST['data']);
						break;
					case 'modify':
						section_modify($_POST['id'], $_POST['data']);
						break;
					case 'remove':
						section_remove($_POST['id']);
						break;
				}
			}
		}
		...
		if (count($routes) === 2){
			if ($_SERVER['REQUEST_METHOD'] === 'GET') {
				landmark_get(json_decode($_GET['filter'], true), json_decode($_GET['options'], true));
			} else if ($_SERVER['REQUEST_METHOD'] === 'POST') {
				switch ($routes[1]) {
					case 'add':
						landmark_add($_POST['data']);
						break;
					case 'modify':
						landmark_modify($_POST['id'], $_POST['data']);
						break;
					case 'remove':
						landmark_remove($_POST['id']);
						break;
				}
			}
		}
		...
		if (count($routes) === 2){
			if ($_SERVER['REQUEST_METHOD'] === 'GET') {
				file_get(json_decode($_GET['filter'], true), json_decode($_GET['options'], true));
			} else if ($_SERVER['REQUEST_METHOD'] === 'POST') {
				switch ($routes[1]) {
					case 'add':
						file_add($_POST['data']);
